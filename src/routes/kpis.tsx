...
const KPIS_KEY = "kpis_unlocked";
const KPIS_PWD = "Carla777";

function KpisGated() {
  const [unlocked, setUnlocked] = useState(() => localStorage.getItem(KPIS_KEY) === "1");
  const [input, setInput] = useState("");
  const [error, setError] = useState(false);

  if (unlocked) return <Kpis />;

  function attempt() {
    if (input === KPIS_PWD) {
      localStorage.setItem(KPIS_KEY, "1");
      setUnlocked(true);
    } else {
      setError(true);
      setInput("");
